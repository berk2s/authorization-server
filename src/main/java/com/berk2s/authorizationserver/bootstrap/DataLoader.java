...
public class DataLoader implements CommandLineRunner {
    ...
    private void loadClients() throws URISyntaxException {
        ...
        if (!userRepository.isEmailTaken("email")) {
            ...
            authority3.setAuthorityName("DELETE_BASKET");
            authority3.addUsers(user);

            Authority authority4 = new Authority();
            authority4.setAuthorityName("UPDATE_BASKET");
            authority4.addUsers(user);

            Authority authority5 = new Authority();
            authority5.setAuthorityName("READ_COURSE");
            authority5.addUsers(user);

            Authority authority6 = new Authority();
            authority6.setAuthorityName("WRITE_COURSE");
            authority6.addUsers(user);

            Authority authority7 = new Authority();
            authority7.setAuthorityName("UPDATE_COURSE");
            authority7.addUsers(user);

            Authority authority8 = new Authority();
            authority8.setAuthorityName("DELETE_COURSE");
            authority8.addUsers(user);

            Role role = new Role();
            role.setRoleName("USER");
            role.addUser(user);

            authorityRepository.saveAll(Set.of(authority,
                    authority2,
                    authority3,
                    authority4,
                    authority5,
